...
namespace GaussianSplatting.Editor.Utils
{
    ...
    {
        public static void ReadFileHeader(string filePath, out int vertexCount, out int vertexStride, out List<(string, ElementType)> attributes)
        {
            vertexCount = 0;
            vertexStride = 0;
            ...
            ReadHeaderImpl(filePath, out vertexCount, out vertexStride, out attributes, fs);
        }

        static void ReadHeaderImpl(string filePath, out int vertexCount, out int vertexStride, out List<(string, ElementType)> attributes, FileStream fs)
        {
            ...
            UnityEngine.Debug.Log($"PLY {filePath} vtx {vertexCount} stride {vertexStride} attrs #{attributes.Count} {string.Join(',', attributes)}");
        }

        public static void ReadFile(string filePath, out int vertexCount, out int vertexStride, out List<(string, ElementType)> attributes, out NativeArray<byte> vertices)
        {
            using var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read);
            ReadHeaderImpl(filePath, out vertexCount, out vertexStride, out attributes, fs);
