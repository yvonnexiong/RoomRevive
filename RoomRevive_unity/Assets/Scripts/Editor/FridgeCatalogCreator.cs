...
public class FridgeCatalogCreator : MonoBehaviour
{
#if UNITY_EDITOR
    ...
    private void LogAttachedObjectAndParents(string reason)
    {
        ...
        sb.AppendLine($"<b>[FridgeCatalogCreator]</b> This script is attached to a GameObject.");
    ...
    [MenuItem("XRCC/Furniture/Find Attached FridgeCatalogCreator In Loaded Objects")]
    public static void FindAttachedInstancesInLoadedObjects()
    {
        FridgeCatalogCreator[] instances = Resources.FindObjectsOfTypeAll<FridgeCatalogCreator>();

        if (instances == null || instances.Length == 0)
        {
            Debug.LogWarning("[FridgeCatalogCreator] No loaded instances found. It may be inside a prefab/scene that is not currently open.");
            return;
        }

        Debug.Log($"[FridgeCatalogCreator] Found {instances.Length} loaded instance(s).");

        foreach (FridgeCatalogCreator instance in instances)
        {
            if (instance == null)
            {
                continue;
    ...
    [MenuItem("XRCC/Furniture/Find References To FridgeCatalogCreator GUID")]
    public static void FindReferencesToThisScriptGuid()
    {
        string[] scriptGuids = AssetDatabase.FindAssets("FridgeCatalogCreator t:MonoScript");

        if (scriptGuids == null || scriptGuids.Length == 0)
        {
            Debug.LogError("[FridgeCatalogCreator] Could not find this script as a MonoScript asset.");
            return;
        }
        ...
        foreach (string scriptGuid in scriptGuids)
        {
            ...
            if (!scriptPath.EndsWith("FridgeCatalogCreator.cs"))
            {
                continue;
            }

            Debug.Log($"[FridgeCatalogCreator] Searching for GUID references:\nGUID: {scriptGuid}\nScript: {scriptPath}");
            ...
            foreach (string fullPath in allFiles)
            {
                ...
                Debug.LogWarning(
                    $"<b>[FridgeCatalogCreator]</b> Found reference to this script GUID in:\n{assetPath}",
                    asset
                );
        ...
        if (totalMatches == 0)
        {
            Debug.LogWarning("[FridgeCatalogCreator] No .unity, .prefab, or .asset files contained this script GUID.");
        }
        else
        {
            Debug.LogWarning($"[FridgeCatalogCreator] Total GUID references found: {totalMatches}");
        }
    }
    ...
    public static void CreateFridgeCatalog()
    {
        ...
        MetaRayFurnitureProductCatalog catalog = AssetDatabase.LoadAssetAtPath<MetaRayFurnitureProductCatalog>(CatalogAssetPath);
        ...
        for (int i = 0; i < catalog.products.Count; i++)
        {
            ...
            {
                Debug.LogWarning(
                    $"[FridgeCatalogCreator] No image found for '{product.productName}'.\n" +
                    $"Place an image in:\n{ImageFolder}\n" +
        ...
        Debug.Log($"<b>[FridgeCatalogCreator]</b> Created/updated fridge catalog:\n{CatalogAssetPath}");
    }
    ...
    private static Sprite DownloadImageAsSprite(string imageUrl, string imageAssetPath)
    {
        ...
        Debug.Log($"[FridgeCatalogCreator] Downloading image:\n{imageUrl}");
        ...
        {
            // Editor blocking download.
            // Fine for a small one-click setup tool.
        }
        ...
        if (hasError)
        {
            Debug.LogWarning($"[FridgeCatalogCreator] Could not download image:\n{imageUrl}\n{request.error}");
            return null;
        }
        ...
        {
            Debug.LogWarning($"[FridgeCatalogCreator] Downloaded image was empty:\n{imageUrl}");
            return null;
        }
    ...
    private static Sprite ImportImageAsSprite(string imageAssetPath)
    {
        ...
        if (sprite == null)
        {
            Debug.LogWarning($"[FridgeCatalogCreator] Could not import image as Sprite:\n{imageAssetPath}");
        }
