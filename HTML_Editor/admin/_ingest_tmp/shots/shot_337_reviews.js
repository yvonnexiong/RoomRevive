...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    card.querySelector('.details').click();
  });
  await sleep(2200);
  // clip from the "Reviews" section heading through the last review
  const box = await page.evaluate(() => {
    const secs = [...document.querySelectorAll('.kp-sec')];
    const rsec = secs.find(s => s.textContent.trim() === 'Reviews');
    const revs = document.querySelector('.kp-revs');
    if (!rsec || !revs) return null;
    const a = rsec.getBoundingClientRect(), b = revs.getBoundingClientRect();
    return { x: a.left - 14, y: a.top - 6, w: (b.right - a.left) + 28, h: (b.bottom - a.top) + 12 };
  });
  const file = path.join(__dirname, 'Kitchen_337_reviews.png');
  await page.screenshot({ path: file, clip: { x: box.x, y: box.y, width: box.w, height: box.h } });
  console.log('SHOT reviews', file);
  await browser.close();
