...
(async () => {
  ...
  const opened = await page.evaluate(() => {
    ...
    const d = card.querySelector('.details'); if (!d) return false; d.click(); return true;
  });
  await sleep(2500);
  const file = path.join(__dirname, 'Kitchen_337_detail.png');
  await page.screenshot({ path: file, fullPage: true });
  console.log('SHOT detail opened=' + opened, file);
  await browser.close();
