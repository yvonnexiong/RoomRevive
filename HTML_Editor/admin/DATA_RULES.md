...
### 2026-06-27 — Ovens (25): full data + English copy + EUR price + 3 mock reviews each
- All 25 filled end-to-end → validate.js Ovens 25/25/25/25/25/25, 0 dupes, 0 type errors. No schema changes needed (reused capacityL/totalPowerKw/weightKg/microwavePowerW/grill/energyClassNote/dimensionsNote/_price*/_copy*).
- **Category page bot-block was harder this time: even `r.jina.ai` returned 403 on the `/category/1022125/ovne` listing.** Worked around it with `WebSearch "Miele <model> <colour> site:miele.dk /product/"` — that reliably returns the `/product/<material>/…` URL (= material number = sku) per model. Got all 25 material numbers + DKK prices this way. (The graphite 125-Edition price wasn't on any reachable miele.dk page → used retailer aahh.dk 13095 kr, marked `_priceQuality:best-effort`, `_priceSource`.)
- **NEW recipe (faster than WebFetch+Read for a big batch): download datasheets via Python `urllib` and extract text with PyMuPDF (`fitz`), not one-PDF-per-Read.** Gotchas: (1) miele.com TLS chain fails default verify → `ssl.create_default_context(); ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE`. (2) datasheet label lines use `\xa0` (nbsp) and `–`; normalise before matching (`Energieffektivitetsklasse (A+++ - D)`, `Nichehøjde min. i mm`, etc.). (3) Console redirect `> file` mangles UTF-8 — write the JSON from Python with `encoding='utf-8'`, don't pipe. fitz is installed; pdfplumber/pypdf are NOT.
- **Oven spec facts:** full ovens (B/BP/BPX) carry EU energy class A+ (A++ for the BPX *compact* ovens w/o microwave, 49 L). **Compact ovens WITH microwave (BM/BMX) have NO EU oven energy label** → `energyClass:null` + `energyClassNote` (same pattern as cooktops/coffee). Datasheet gives build-in *niche* dims, not external — stored as "H x W x D mm (niche)" + `dimensionsNote` (appliance front ~595 mm). Marketing class ladder Silver→Gold→Platinum→Diamond maps to tiers; note DK sheets sometimes spell it "Diamant".
- Wrote via PUT /api/items/:id {product:{…}} (single writer, server up), throttled 0.4s + EBUSY/5xx backoff retry — 25/25 OK, no EBUSY hit. Backup at admin/catalog.backup-ovens.json.
